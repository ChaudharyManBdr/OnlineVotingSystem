 ...
 * @author Manav
 */
@WebServlet(name = "WinnerResultShow", urlPatterns = {"/WinnerResultShow"})
public class WinnerResultShow extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (partyName == null) {
            ...
            list = mrs.showWinnerResultByDistrict();
        } else if (districtName == null) {
            mrs.setParty(partyName);
            list = mrs.showWinnerResultByParty();
        } else if (districtName==null && partyName==null) {
        ...
        if (list != null) {
            ...
            RequestDispatcher view = request.getRequestDispatcher("winnerList.jsp");
            view.forward(request, response);
        } else {
            request.setAttribute("Message", "Error : Sorry Failed !");
            RequestDispatcher view = request.getRequestDispatcher("messageError.jsp");
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet WinnerResultShow</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet WinnerResultShow at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
